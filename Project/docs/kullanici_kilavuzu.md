...
| Seçenek | Kısa Form | Açıklama | Örnek |
|---------|-----------|-----------|--------|
| `--file` | `-f` | Süreç verilerini içeren CSV dosyası yolu | `-f data/surecler.csv` |
| `--both` | `-b` | Her iki test senaryosunu çalıştır (case1.csv ve case2.csv) | `--both` |
| `--threading` | `-t` | Paralel yürütme için multi-threading kullan | `--threading` |
| `--context-switch` | `-c` | Bağlam değiştirme süresini zaman birimi cinsinden ayarla (varsayılan: 0.001) | `-c 0.002` |
| `--help` | `-h` | Yardım mesajını göster | `--help` |
...
Bu simülatör eğitim amaçlı tasarlanmıştır ve şu konular için mükemmeldir:
...
**Destek**: Herhangi bir sorun veya sorunuz için lütfen ilgili belgeleri kontrol edin veya teknik yardım isteyin.